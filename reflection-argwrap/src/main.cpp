...
{
public:
    void PassByValue(std::string s) const { std::cout << "Foo::PassByValue(`" << s << "`)" << std::endl; }
...
void TestMemberVariable()
{
    ...
    std::cout << "s.a=" << s_a.GetValue<int>(s) << " expected=" << s.a << std::endl;
    std::cout << "s.b=" << s_b.GetValue<float>(s) << " expected=" << s.b << std::endl;
...
void TestFoo()
{
    ...
    {
        std::cout << "member func: " << mf.name() << ", is_const=" << mf.is_const() << std::endl;
    }
    std::cout << std::endl;
